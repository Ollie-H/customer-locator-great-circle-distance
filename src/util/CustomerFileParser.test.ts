...
const mockExpectedCompany = new Company('47a83870-994d-4741-b169-da4651c5b4c2', 51.00189352, 18.19409139);
...
const parloaFindCustomersWithinDistanceSpy = jest
  .spyOn(parloa, 'findCustomersWithinDistance')
  .mockImplementation(() => [mockExpectedCompany]);
...
describe('CustomerFileParser', () => {
  ...
  test('inviteCustomers logs matched customers', () => {
    ...
    expect(mockLogMethod).toBeCalledWith(
      `${mockExpectedCompany.id}: Invite customer ${mockExpectedCompany.id} to company anniversary, distace to Paloa (km): ${mockExpectedCompany.distanceToParloa}`,
    );
  });
});
